...
- [Honeycomb](#honeycomb)
    ...
    - [Examples](#examples)
        ...
        - [Grids extend `Array.prototype`](#grids-extend-arrayprototype)
            - [Grid methods that mutate](#grid-methods-that-mutate)
            - [Be careful with bracket notation](#be-careful-with-bracket-notation)
        - [Point → Hex](#point-%E2%86%92-hex)
    ...
    - [Backlog](#backlog)
        ...
        - [🛠 Refactorings](#%F0%9F%9B%A0-refactorings)
...
1.  Add src to package and point `module` to it? Also point `main` to unminified version by default? Also: add `browser` property to package.json? See [jimp](https://github.com/oliver-moran/jimp) for an example of a library that's used on the server and the browser.
2.  Hex methods that do nothing with a hex's coordinates should be static (e.g. `cubeToCartesian`, `isPointy`, `width`)?
...
5.  Instead of filtering invalid hexes, attempt to convert values to hexes (by passing them to `Hex()`)?
